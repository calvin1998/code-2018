...
try
    ...
    msg = 'Could not open port. '...
          'Check if in use by Arduino IDE.' ...
          'May need to restart MATLAB.';
    error(msg)
end

%% Create data fields
modA = []; modB = []; modC = []; gate = [];
Motor_Temp = []; Motor_Speed = 0; Motor_Angle = [];
A = []; B = []; C = []; DC_I = [];
DC_V = []; Out = []; AB = []; BC = [];
...
while run
    ...
    elseif strcmp(id,'ID_MC_TEMPERATURES_3')
        vals = vals/10;
        Motor_Temp = [Motor_Temp vals(3)];
        subplot(3,3,8), hold on,
        plot(Motor_Temp,'k');
        title(temp3)
        if length(Motor_Temp) < L
            axis([0 L 0 100]);
        else
            axis([length(Motor_Temp)-L length(Motor_Temp) 0 100]);
        end
